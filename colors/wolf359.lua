-- wolf359.nvim — dark colorscheme optimized for Rust / Neovim 0.12+
...
hi("NormalFloat",       { fg = c.fg0,    bg = c.bg2 })
hi("FloatBorder",       { fg = c.blue3,  bg = c.bg2 })
...
hi("LineNr",            { fg = c.steel2, bg = c.bg1 })
hi("SignColumn",        { bg = c.bg1 })
hi("ColorColumn",       { bg = c.bg4 })
hi("FoldColumn",        { fg = c.steel2, bg = c.bg1 })
hi("Folded",            { fg = c.steel,  bg = c.bg3, italic = true })

hi("Visual",            { bg = c.bg_sel })
hi("VisualNOS",         { bg = c.bg3 })

hi("Search",            { fg = c.gold,   bg = c.orange3 })
...
hi("MatchParen",        { fg = c.gold,   bg = c.orange3, bold = true })
hi("NonText",           { fg = c.bg5 })
...
--  DIFF
-- ════════════════════════════════════════════════════════════
hi("DiffAdd",           { fg = c.green,  bg = "#0C1A10" })
...
hi("@function.call",            { fg = "#A07A18" })
hi("@function.method",          { fg = c.gold, bold = true })
hi("@function.method.call",     { fg = "#D0AA28" })
hi("@function.macro",           { fg = c.red, bold = true })        -- println!, vec!, panic!
...
hi("@keyword.import",           { fg = c.orange })                  -- use
...
hi("@comment",                  { fg = c.fg2, italic = true })
hi("@comment.documentation",    { fg = "#5A7888", italic = true })
...
hi("@module",                   { fg = "#506880" })
hi("@module.builtin",           { fg = "#506880" })
hi("@namespace",                { fg = "#506880" })

hi("@attribute",                { fg = c.red })
hi("@attribute.builtin",        { fg = c.red })

hi("@label",                    { fg = c.copper, italic = true })

hi("@markup.heading",           { fg = c.orange, bold = true })
hi("@markup.link",              { fg = c.blue, underline = true })
hi("@markup.raw",               { fg = "#B08828" })
...
hi("@lsp.type.field",            { fg = c.fg0 })
hi("@lsp.type.variable",         { link = "@variable" })
hi("@lsp.type.parameter",        { fg = c.fg1 })
hi("@lsp.type.macro",            { fg = c.red, bold = true })
...
hi("DiagnosticVirtualTextWarn",  { fg = c.gold2,  italic = true })
hi("DiagnosticVirtualTextInfo",  { fg = c.blue2,  italic = true })
hi("DiagnosticVirtualTextHint",  { fg = c.cyan2,  italic = true })
...
hi("LspInlayHint",                 { fg = c.steel2, bg = c.bg2, italic = true })
...
local lsp_types = {
  "function", "method", "type", "class", "struct", "enum", "enumMember",
  "interface", "typeParameter", "property", "field", "variable", "parameter",
  "macro", "lifetime", "keyword", "comment", "string", "number", "boolean",
  "selfKeyword", "selfTypeKeyword", "formatSpecifier", "attribute", "decorator",
  "namespace", "builtinType", "generic",
}

local lsp_modifiers = {
  "declaration", "definition", "static", "abstract", "deprecated",
  "readonly", "defaultLibrary", "async", "attribute", "callable",
  "constant", "consuming", "controlFlow", "crateRoot", "injected",
  "library", "macro", "mutable", "public", "reference", "trait",
  "unsafe", "documentation", "intraDocLink", "derive",
}
...
do
  ...
  local function hl_derive_traits(bufnr)
    ...
    if not _dq then
      local ok_q, q = pcall(vim.treesitter.query.parse, "rust", [[
        (attribute_item
          (attribute
            (identifier) @_name
      ...
      if ok_q then _dq = q end
    end
    if not _dq then return end
  ...
  vim.api.nvim_create_autocmd("LspTokenUpdate", {
    ...
    callback = function(args)
      ...
      if t.type == "struct" and t.modifiers.defaultLibrary then
        htok(t, args.buf, args.data.client_id,
          "@lsp.typemod.struct.defaultLibrary", { priority = prio })
      elseif t.type == "interface" then
        htok(t, args.buf, args.data.client_id,
          "@lsp.type.interface", { priority = prio })
      end
    end,
  })
  ...
  vim.api.nvim_create_autocmd({ "BufEnter", "BufRead", "BufWritePost" }, {
    ...
    pattern = "*.rs",
    callback = function(ev)
      vim.schedule(function() hl_derive_traits(ev.buf) end)
    end,
  })
